...
import { executeArbitrage } from '../src/trading/execution/arbitrage-executor';
import { executeExoticArbitrage } from '../src/trading/execution/exotic-arbitrage-executor';
config();
class ArbitrageLoopController {
    ...
    async executeArbitrageRun() {
        ...
        try {
            ...
            if (this.config.mode === 'triangular' || this.config.mode === 'cross-pair' || this.config.mode === 'exotic-hunt') {
                ...
                const exoticConfig = {
                    mode: this.config.mode === 'exotic-hunt' ? 'hunt-execute' : this.config.mode,
                ...
                const result = await executeExoticArbitrage(exoticConfig);
                if (result.success) {
    ...
    isRateLimitError(errorMessage) {
        ...
        return rateLimitIndicators.some(indicator => errorMessage.toLowerCase().includes(indicator.toLowerCase()));
    }
    calculateDelay() {
        // Base delay
...
//# sourceMappingURL=arbitrage-loop.js.map