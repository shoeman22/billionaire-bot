...
import { executeExoticArbitrage, discoverTriangularOpportunities, discoverCrossPairOpportunities, huntAndExecuteArbitrage } from '../../src/trading/execution/exotic-arbitrage-executor';
config();
/**
 ...
 */
function displayOpportunities(opportunities, title) {
 ...
 */
async function executeDiscovery(options) {
 ...
 */
async function executeArbitrageMode(options) {
    const amount = options.amount || TRADING_CONSTANTS.DEFAULT_TRADE_SIZE;
    let threshold;
 ...
 */
async function executeHuntMode(options) {
...
async function main() {
    ...
    if (options.amount)
        logger.info(`   Amount: ${options.amount} GALA`);
    if (options.threshold)
        logger.info(`   Threshold: ${options.threshold}%`);
    if (options.dryRun)
        logger.info(`   Dry run: enabled`);
    if (options.autoExecute)
        logger.info(`   Auto execute: enabled`);
    try {
        switch (options.mode) {
            case 'discover':
...
//# sourceMappingURL=exotic-arbitrage.js.map